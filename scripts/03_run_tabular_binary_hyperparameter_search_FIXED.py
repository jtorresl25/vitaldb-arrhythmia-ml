...
def _make_binary_target(y: np.ndarray) -> np.ndarray:
    """Convierte rhythm_label multiclase a etiqueta binaria para el modelo principal.

    normal   = N
    abnormal = cualquier ritmo distinto de N
    """
    y_str = pd.Series(y).astype(str).str.strip()
    return np.where(y_str.eq("N"), "normal", "abnormal")


def _save_binary_extra_metrics(y_true, y_pred, tables_dir: Path) -> None:
    """Guarda métricas binarias explícitas para la app/reporte académico."""
    ...
    rows = [{
        "tn_normal": int(tn),
        "fp_abnormal_false_alarm": int(fp),
        "fn_abnormal_missed": int(fn),
        "tp_abnormal": int(tp),
        "sensitivity_recall_abnormal": recall_score(y_true, y_pred, pos_label="abnormal", zero_division=0),
        "specificity_recall_normal": recall_score(y_true, y_pred, pos_label="normal", zero_division=0),
        "precision_abnormal": precision_score(y_true, y_pred, pos_label="abnormal", zero_division=0),
        "f1_abnormal": f1_score(y_true, y_pred, pos_label="abnormal", zero_division=0),
        "accuracy": accuracy_score(y_true, y_pred),
    }]
    pd.DataFrame(rows).to_csv(tables_dir / "tabular_binary_metrics.csv", index=False)

    pd.DataFrame(
        [(r, c, int(cm[i, j])) for i, r in enumerate(labels) for j, c in enumerate(labels)],
        columns=["real_label", "predicted_label", "count"],
...
def _extract_feature_importance_binary_safe(estimator) -> pd.DataFrame | None:
    """Extrae importancia de features sin fallar en clasificadores binarios.

    El helper original del proyecto espera, para modelos lineales, una fila de
    coeficientes por clase. En regresión logística binaria sklearn guarda
    coef_ con shape (1, n_features), así que aquí generamos un ranking global
    usando abs(coef) y manteniendo columnas compatibles con la app.
    """
    try:
        pre = estimator.named_steps.get("preprocessor") if hasattr(estimator, "named_steps") else None
        clf = estimator.named_steps.get("clf") if hasattr(estimator, "named_steps") else estimator

        if pre is not None and hasattr(pre, "get_feature_names_out"):
            feature_names = list(pre.get_feature_names_out())
        else:
            feature_names = [f"feature_{i}" for i in range(getattr(clf, "n_features_in_", 0))]

        if hasattr(clf, "feature_importances_"):
            vals = np.asarray(clf.feature_importances_, dtype=float).ravel()
            n = min(len(feature_names), len(vals))
            return (
                pd.DataFrame({"feature": feature_names[:n], "importance": vals[:n]})
        ...
        if hasattr(clf, "coef_"):
            ...
            # Caso binario sklearn: coef_.shape == (1, n_features).
            # Ese vector corresponde a la clase positiva clf.classes_[1].
            if coef.shape[0] == 1:
                positive_class = classes[1] if len(classes) >= 2 else "positive"
                vals = coef[0]
                n = min(len(feature_names), len(vals))
                ...
                n_classes = min(coef.shape[0], len(classes) if classes else coef.shape[0])
                n_feats = min(len(feature_names), coef.shape[1])
                for c_idx in range(n_classes):
                    cls = classes[c_idx] if classes else c_idx
                    for f_idx in range(n_feats):
                        v = float(coef[c_idx, f_idx])
                        rows.append({
                            "class": cls,
                            "feature": feature_names[f_idx],
                            "coef": v,
...
def main() -> int:
    ...
    y_multiclass = df[config.TARGET_COLUMN].to_numpy()
    y = _make_binary_target(y_multiclass)
    groups = df[config.CASE_ID_COLUMN].to_numpy()
    logger.info("Target binario: %s", pd.Series(y).value_counts().to_dict())
    ...
    if winner is None:
        ...
        _save_binary_extra_metrics(y_test, y_pred, tables_dir)
        ...
        # Feature importance
        fi = _extract_feature_importance_binary_safe(best_est)
        if fi is not None:
    ...
    if winner is not None:
        ...
        _model_meta = {
            "winner_model": m_name,
            "task": "binary_normal_vs_abnormal",
            "normal_definition": "rhythm_label == N",
            "abnormal_definition": "rhythm_label != N",
